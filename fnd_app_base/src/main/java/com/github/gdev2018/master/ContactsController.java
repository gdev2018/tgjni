...
public class ContactsController {
            ...
            Utilities.globalQueue.postRunnable(() -> {
                try {
                    if (hasContactsPermission()) {
                        BaseApplication.mApplicationContext.getContentResolver().registerContentObserver(ContactsContract.Contacts.CONTENT_URI, true, new MyContentObserver());
                    }
    ...
    public void checkAppAccount() {
        ...
        try {
            Account[] accounts = am.getAccountsByType("com.github.gdev2018.master");
            systemAccount = null;
        ...
        if (UserConfig.getInstance(currentAccount).isClientActivated()) {
            ...
            if (systemAccount == null) {
                try {
                    systemAccount = new Account("" + UserConfig.getInstance(currentAccount).getClientUserId(), "com.github.gdev2018.master");
    ...
    private HashMap<String, Contact> readContactsFromPhoneBook() {
        ...
        /*if (BaseBuildVars.LOGS_ENABLED && contactsMap != null) {
                ...
                try {
                    ...
                    if (UserConfig.getInstance(currentAccount).isClientActivated()) {
                        if (accounts.length != 1) {
                            ...
                            currentAccount = new Account(UserConfig.getInstance(currentAccount).getCurrentUser().phone, "com.github.gdev2018.master.account");
    ...
    private boolean hasContactsPermission() {
        if (Build.VERSION.SDK_INT >= 23) {
            return BaseApplication.mApplicationContext.checkSelfPermission(android.Manifest.permission.READ_CONTACTS) == PackageManager.PERMISSION_GRANTED;
        }
        Cursor cursor = null;
        try {
            ContentResolver cr = BaseApplication.mApplicationContext.getContentResolver();
    ...
    private void performWriteContactsToPhoneBookInternal(ArrayList<TLRPC.TL_contact> contactsArray) {
        ...
        try {
            ...
            cursor = BaseApplication.mApplicationContext.getContentResolver().query(rawContactUri, new String[]{BaseColumns._ID, ContactsContract.RawContacts.SYNC2}, null, null, null);
...
//        query.add(builder.build());
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.com.github.gdev2018.master.android.profile");
    ...
    public void createOrUpdateConnectionServiceContact(int id, String firstName, String lastName) {
        if (!hasContactsPermission())
            return;
        try {
            ContentResolver resolver = BaseApplication.mApplicationContext.getContentResolver();
