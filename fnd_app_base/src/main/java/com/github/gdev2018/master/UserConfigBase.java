...
//    public TLRPC.TL_help_appUpdate pendingAppUpdate;*/
    ...
    private static volatile UserConfigBase[] Instance = new UserConfigBase[UserConfigBase.MAX_ACCOUNT_COUNT];
    public static UserConfigBase getInstance(int num) {
        UserConfigBase localInstance = Instance[num];
        if (localInstance == null) {
            synchronized (UserConfigBase.class) {
                localInstance = Instance[num];
                if (localInstance == null) {
                    Instance[num] = localInstance = new UserConfigBase(num);
                }
            }
        }
    ...
    public UserConfigBase(int instance) {
    ...
    public void saveConfig(boolean withFile, File oldFile) {
        synchronized (sync) {
            try {
                ...
                if (currentAccount == 0) {
                    preferences = BaseApplication.mApplicationContext.getSharedPreferences("userconfing_base", Context.MODE_PRIVATE);
                } else {
                    preferences = BaseApplication.mApplicationContext.getSharedPreferences("userconfig_base" + currentAccount, Context.MODE_PRIVATE);
                }
...
//                }*/
    ...
    public void loadConfig() {
        synchronized (sync) {
            ...
            SharedPreferences preferences;
            if (currentAccount == 0) {
                preferences = BaseApplication.mApplicationContext.getSharedPreferences("userconfig_base", Context.MODE_PRIVATE);
...
//                            FileLog.e(e);
//                        }
//                        if (pendingAppUpdateBuildVersion != BaseBuildVars.BUILD_VERSION || pendingAppUpdateInstallTime < updateTime) {
