...
//            }*/
    ...
    public static boolean checkApkInstallPermissions(final Context context) {
        if (Build.VERSION.SDK_INT >= 26 && !BaseApplication.mApplicationContext.getPackageManager().canRequestPackageInstalls()) {
            ...
            builder.setPositiveButton(LocaleController.getString("PermissionOpenSettings", R.string.PermissionOpenSettings), (dialogInterface, i) -> {
                try {
                    context.startActivity(new Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES, Uri.parse("package:" + BaseApplication.mApplicationContext.getPackageName())));
                } catch (Exception e) {
                    FileLog.e(e);
                }
            });
