...
import com.github.gdev2018.master.ui.Components.BetterRatingView;
...
public class VoIPHelper {
	...
	public static void startCall(TLRPC.User user, boolean videoCall, boolean canVideoCall, final Activity activity, TLRPC.UserFull userFull, BaseAccountInstance accountInstance) {
		...
		if (ConnectionsManager.getInstance(BaseUserConfig.selectedAccount).getConnectionState() != ConnectionsManager.ConnectionStateConnected) {
	...
	public static void startCall(TLRPC.Chat chat, TLRPC.InputPeer peer, String hash, boolean createCall, Activity activity, BaseFragment fragment, BaseAccountInstance accountInstance) {
		if (activity == null) {
			return;
		}
		if (ConnectionsManager.getInstance(BaseUserConfig.selectedAccount).getConnectionState() != ConnectionsManager.ConnectionStateConnected) {
	...
	private static void initiateCall(TLRPC.User user, TLRPC.Chat chat, String hash, boolean videoCall, boolean canVideoCall, boolean createCall, final Activity activity, BaseFragment fragment, BaseAccountInstance accountInstance) {
...
//					}
//					GroupCallActivity.create((LaunchActivity) activity, BaseAccountInstance.getInstance(BaseUserConfig.selectedAccount), null, null, false, null);
//				}*/
			}
	...
	private static void doInitiateCall(TLRPC.User user, TLRPC.Chat chat, String hash, TLRPC.InputPeer peer, boolean hasFewPeers, boolean videoCall, boolean canVideoCall, boolean createCall, Activity activity, BaseFragment fragment, BaseAccountInstance accountInstance, boolean checkJoiner, boolean checkAnonymous) {
		if (activity == null || user == null && chat == null) {
			return;
		}
...
//			}*/
		...
		intent.putExtra("account", BaseUserConfig.selectedAccount);
		try {
	...
	public static File getLogsDir() {
		File logsDir = new File(BaseApplication.mApplicationContext.getCacheDir(), "voip_logs");
		if (!logsDir.exists()) {
			logsDir.mkdirs();
		}
		return logsDir;
	...
	public static boolean canRateCall(TLRPC.TL_messageActionPhoneCall call) {
		if (!(call.reason instanceof TLRPC.TL_phoneCallDiscardReasonBusy) && !(call.reason instanceof TLRPC.TL_phoneCallDiscardReasonMissed)) {
			SharedPreferences prefs = MessagesController.getNotificationsSettings(BaseUserConfig.selectedAccount); // always called from chat UI
		...
		for (String hash : hashes) {
			...
			if (d[0].equals(call.call_id + "")) {
				try {
					...
					showRateAlert(context, null, call.video, call.call_id, accessHash, BaseUserConfig.selectedAccount, true);
	...
	public static void showRateAlert(final Context context, final Runnable onDismiss, boolean isVideo, final long callID, final long accessHash, final int account, final boolean userInitiative) {
		...
		final AlertDialog alert = new AlertDialog.Builder(context)
				...
				.create();
		if (BaseBuildVars.LOGS_ENABLED && log.exists()) {
			...
			if (rating >= 4 || page[0] == 1) {
				...
				ConnectionsManager.getInstance(account).sendRequest(req, (response, error) -> {
					...
					if (includeLogs[0] && log.exists() && req.rating < 4) {
						BaseAccountInstance accountInstance = BaseAccountInstance.getInstance(BaseUserConfig.selectedAccount);
///*
...
//*/
	...
	private static File getLogFile(long callID) {
		if (BaseBuildVars.DEBUG_VERSION) {
			File debugLogsDir = new File(BaseApplication.mApplicationContext.getExternalFilesDir(null), "logs");
	...
	public static void showCallDebugSettings(final Context context) {
		...
		if (BaseBuildVars.DEBUG_VERSION && BaseBuildVars.LOGS_ENABLED) {
	...
	public static String getLogFilePath(String name) {
		...
		final File externalFilesDir = BaseApplication.mApplicationContext.getExternalFilesDir(null);
	...
	public static void showGroupCallAlert(BaseFragment fragment, TLRPC.Chat currentChat, TLRPC.InputPeer peer, boolean recreate, BaseAccountInstance accountInstance) {
