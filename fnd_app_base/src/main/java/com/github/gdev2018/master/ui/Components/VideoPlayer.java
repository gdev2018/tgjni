...
import com.github.gdev2018.master.FourierTransform;
...
import com.github.gdev2018.master.secretmedia.ExtendedDefaultDataSourceFactory;
...
public class VideoPlayer implements ExoPlayer.EventListener, SimpleExoPlayer.VideoListener, AnalyticsListener, NotificationCenter.NotificationCenterDelegate {
    ...
    public VideoPlayer(boolean pauseOther) {
        mediaDataSourceFactory = new ExtendedDefaultDataSourceFactory(BaseApplication.mApplicationContext, BANDWIDTH_METER, new DefaultHttpDataSourceFactory("Mozilla/5.0 (X11; Linux x86_64; rv:10.0) Gecko/20150101 Firefox/47.0 (Chrome)", BANDWIDTH_METER));
    ...
    private void ensurePleyaerCreated() {
        ...
        if (player == null) {
            ...
            if (audioVisualizerDelegate != null) {
                factory = new AudioVisualizerRenderersFactory(BaseApplication.mApplicationContext);
            } else {
                factory = new DefaultRenderersFactory(BaseApplication.mApplicationContext);
            }
            factory.setExtensionRendererMode(DefaultRenderersFactory.EXTENSION_RENDERER_MODE_PREFER);
            player = ExoPlayerFactory.newSimpleInstance(BaseApplication.mApplicationContext, factory, trackSelector, loadControl, null);
        ...
        if (mixedAudio) {
            if (audioPlayer == null) {
                audioPlayer = ExoPlayerFactory.newSimpleInstance(BaseApplication.mApplicationContext, trackSelector, loadControl, null, DefaultRenderersFactory.EXTENSION_RENDERER_MODE_PREFER);
