...
public class StatsController {
    ...
    private static volatile StatsController[] Instance = new StatsController[BaseUserConfig.MAX_ACCOUNT_COUNT];
    ...
    private StatsController(int account) {
        ...
        if (account != 0) {
            filesDir = new File(BaseApplication.getFilesDirFixed(), "account" + account + "/");
            filesDir.mkdirs();
        }
        ...
        if (needConvert) {
            ...
            if (account == 0) {
                sharedPreferences = BaseApplication.mApplicationContext.getSharedPreferences("stats", Context.MODE_PRIVATE);
            } else {
                sharedPreferences = BaseApplication.mApplicationContext.getSharedPreferences("stats" + account, Context.MODE_PRIVATE);
            }
            boolean save = false;
            for (int a = 0; a < 3; a++) {
