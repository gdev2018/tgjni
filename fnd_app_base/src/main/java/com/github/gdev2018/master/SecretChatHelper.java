...
import com.github.gdev2018.master.tgnet.AbstractSerializedData;
import com.github.gdev2018.master.tgnet.ConnectionsManager;
import com.github.gdev2018.master.tgnet.NativeByteBuffer;
import com.github.gdev2018.master.tgnet.TLClassStore;
...
///*                ImageLoader.getInstance().replaceImageInCache(fileName, fileName2, size.location, true);*/
        ...
        Utilities.stageQueue.postRunnable(() -> {
            try {
                ...
                if (chat.seq_in == 0 && chat.seq_out == 0) {
                    if (chat.admin_id == BaseUserConfig.getInstance(currentAccount).getClientUserId()) {
                        chat.seq_out = 1;
                        chat.seq_in = -2;
                    } else {
                        chat.seq_in = -1;
                ...
                byte[] messageKeyFull;
                boolean incoming = mtprotoVersion == 2 && chat.admin_id != BaseUserConfig.getInstance(currentAccount).getClientUserId();
                if (mtprotoVersion == 2) {
    ...
    public TLRPC.Message processDecryptedObject(final TLRPC.EncryptedChat chat, final TLRPC.EncryptedFile file, int date, TLObject object, boolean new_key_used) {
        if (object != null) {
            ...
            if (from_id == BaseUserConfig.getInstance(currentAccount).getClientUserId()) {
...
///*                    newMessage.local_id = newMessage.id = BaseUserConfig.getInstance(currentAccount).getNewMessageId();*/
    ...
    private void resendMessages(final int startSeq, final int endSeq, final TLRPC.EncryptedChat encryptedChat) {
        ...
        MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() -> {
            try {
                int sSeq = startSeq;
                if (encryptedChat.admin_id == BaseUserConfig.getInstance(currentAccount).getClientUserId() && sSeq % 2 == 0) {
                    sSeq++;
                }
...
///*                        messages.add(createDeleteMessage(BaseUserConfig.getInstance(currentAccount).getNewMessageId(), messagesToResend.keyAt(a), 0, Utilities.random.nextLong(), encryptedChat));*/
                    }
                    BaseUserConfig.getInstance(currentAccount).saveConfig(false);
                }
    ...
    protected ArrayList<TLRPC.Message> decryptMessage(TLRPC.EncryptedMessage message) {
        ...
        try {
            ...
            if (keyToDecrypt != null) {
                ...
                boolean incoming = chat.admin_id == BaseUserConfig.getInstance(currentAccount).getClientUserId();
