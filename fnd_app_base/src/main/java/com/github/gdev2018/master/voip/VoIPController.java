...
public class VoIPController {
	...
	public VoIPController() {
		nativeInst = nativeInit(new File(BaseApplication.mApplicationContext.getFilesDir(), "voip_persistent_state.json").getAbsolutePath());
	}
	...
	public void setConfig(double recvTimeout, double initTimeout, int dataSavingOption, long callID) {
		...
		nativeSetConfig(nativeInst, recvTimeout, initTimeout, dataSavingOption,
				...
				true, BaseBuildVars.DEBUG_VERSION ? getLogFilePath("voip" + callID) : getLogFilePath(callID), BaseBuildVars.DEBUG_VERSION && dump ? getLogFilePath("voipStats") : null,
				BaseBuildVars.DEBUG_VERSION);
	}
	...
	private String getLogFilePath(String name) {
		...
		return new File(BaseApplication.mApplicationContext.getExternalFilesDir(null), String.format(Locale.US, "logs/%02d_%02d_%04d_%02d_%02d_%02d_%s.txt", c.get(Calendar.DATE), c.get(Calendar.MONTH) + 1, c.get(Calendar.YEAR), c.get(Calendar.HOUR_OF_DAY), c.get(Calendar.MINUTE), c.get(Calendar.SECOND), name)).getAbsolutePath();
	}
