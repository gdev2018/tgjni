...
public class DataQuery {
    ...
    public DataQuery(int num) {
        ...
        if (currentAccount == 0) {
            preferences = BaseApplication.mApplicationContext.getSharedPreferences("drafts", Activity.MODE_PRIVATE);
        } else {
            preferences = BaseApplication.mApplicationContext.getSharedPreferences("drafts" + currentAccount, Activity.MODE_PRIVATE);
        }
    ...
    public void addRecentSticker(final int type, Object parentObject, TLRPC.Document document, int date, boolean remove) {
        ...
        if (type == TYPE_FAVE) {
            if (remove) {
                Toast.makeText(BaseApplication.mApplicationContext, LocaleController.getString("RemovedFromFavorites", R.string.RemovedFromFavorites), Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(BaseApplication.mApplicationContext, LocaleController.getString("AddedToFavorites", R.string.AddedToFavorites), Toast.LENGTH_SHORT).show();
            }
...
//                            baseFragment.showDialog(alert.create());*/
    ...
    public void buildShortcuts() {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            try {
                ShortcutManager shortcutManager = BaseApplication.mApplicationContext.getSystemService(ShortcutManager.class);
...
///*                Intent intent = new Intent(BaseApplication.mApplicationContext, LaunchActivity.class);*/
                ...
                arrayList.add(new ShortcutInfo.Builder(BaseApplication.mApplicationContext, "compose")
                        ...
                        .setIcon(Icon.createWithResource(BaseApplication.mApplicationContext, R.drawable.shortcut_compose))
                        .setIntent(intent)
                        .build());
                ...
                for (int a = 0; a < hintsFinal.size(); a++) {
                    Intent shortcutIntent = new Intent(BaseApplication.mApplicationContext, OpenChatReceiver.class);
                    TLRPC.TL_topPeer hint = hintsFinal.get(a);
                    ...
                    ShortcutInfo.Builder builder = new ShortcutInfo.Builder(BaseApplication.mApplicationContext, id)
                            .setShortLabel(name)
                            .setLongLabel(name)
                    ...
                    if (bitmap != null) {
                        ...
                        builder.setIcon(Icon.createWithResource(BaseApplication.mApplicationContext, R.drawable.shortcut_user));
                    }
        ...
        try {
            ...
            if (selfUser || photo != null) {
                try {
                    ...
                    if (selfUser || bitmap != null) {
                        ...
                        Drawable drawable = BaseApplication.mApplicationContext.getResources().getDrawable(R.drawable.book_logo);
            ...
            if (Build.VERSION.SDK_INT >= 26) {
                ShortcutInfo.Builder pinShortcutInfo =
                        new ShortcutInfo.Builder(BaseApplication.mApplicationContext, "sdid_" + did)
                                .setShortLabel(name)
                                .setIntent(shortcutIntent);
                ...
                if (bitmap != null) {
                    ...
                    if (user != null) {
                        if (user.bot) {
                            pinShortcutInfo.setIcon(Icon.createWithResource(BaseApplication.mApplicationContext, R.drawable.book_bot));
                        } else {
                            pinShortcutInfo.setIcon(Icon.createWithResource(BaseApplication.mApplicationContext, R.drawable.book_user));
                        }
                    } else if (chat != null) {
                        if (ChatObject.isChannel(chat) && !chat.megagroup) {
                            pinShortcutInfo.setIcon(Icon.createWithResource(BaseApplication.mApplicationContext, R.drawable.book_channel));
                        } else {
                            pinShortcutInfo.setIcon(Icon.createWithResource(BaseApplication.mApplicationContext, R.drawable.book_group));
                        }
                    }
                }
                ...
                if (bitmap != null) {
                    ...
                    if (user != null) {
                        if (user.bot) {
                            addIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE, Intent.ShortcutIconResource.fromContext(BaseApplication.mApplicationContext, R.drawable.book_bot));
                        } else {
                            addIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE, Intent.ShortcutIconResource.fromContext(BaseApplication.mApplicationContext, R.drawable.book_user));
                        }
                    } else if (chat != null) {
                        if (ChatObject.isChannel(chat) && !chat.megagroup) {
                            addIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE, Intent.ShortcutIconResource.fromContext(BaseApplication.mApplicationContext, R.drawable.book_channel));
                        } else {
                            addIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE, Intent.ShortcutIconResource.fromContext(BaseApplication.mApplicationContext, R.drawable.book_group));
                        }
                    }
                }
                ...
                BaseApplication.mApplicationContext.sendBroadcast(addIntent);
            }
        } catch (Exception e) {
            FileLog.e(e);
        }
