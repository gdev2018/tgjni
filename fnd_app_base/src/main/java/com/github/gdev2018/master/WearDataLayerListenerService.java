...
public class WearDataLayerListenerService extends WearableListenerService {
		...
		try {
			if ("/getCurrentUser".equals(path)) {
				...
				if (BaseUserConfig.getInstance(currentAccount).isClientActivated()) {
					final TLRPC.User user = BaseUserConfig.getInstance(currentAccount).getCurrentUser();
				...
				try{
					...
					for(int i = 0; i< BaseUserConfig.getActivatedAccountsCount(); i++){
						if(BaseUserConfig.getInstance(i).getClientUserId()==accountID){
							currentAccount=i;
							break;
						}
					}
	...
	public static void sendMessageToWatch(final String path, final byte[] data, String capability){
		Wearable.getCapabilityClient(BaseApplication.mApplicationContext)
				...
				.addOnCompleteListener(new OnCompleteListener<CapabilityInfo>(){
					...
					public void onComplete(@NonNull Task<CapabilityInfo> task){
						...
						if(info!=null){
							MessageClient mc=Wearable.getMessageClient(BaseApplication.mApplicationContext);
