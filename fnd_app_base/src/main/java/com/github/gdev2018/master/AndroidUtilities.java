...
import com.github.gdev2018.master.ui.WallpapersListActivity;
...
public class AndroidUtilities {
    ...
    static {
        ...
        checkDisplaySize(BaseApplication.mApplicationContext, null);
    }
    ...
    public static boolean isGoogleMapsInstalled(final BaseFragment fragment) {
        try {
            BaseApplication.mApplicationContext.getPackageManager().getApplicationInfo("com.google.android.apps.maps", 0);
            return true;
    ...
    private static boolean isInternalUri(Uri uri, int fd) {
        ...
        if (uri != null) {
            ...
            if (pathString.matches(Pattern.quote(new File(BaseApplication.mApplicationContext.getCacheDir(), "voip_logs").getAbsolutePath()) + "/\\d+\\.log")) {
                return false;
            }
            int tries = 0;
            while (true) {
        ...
        return pathString != null && pathString.toLowerCase().contains("/data/data/" + BaseApplication.mApplicationContext.getPackageName());
    }
    ...
    public static ArrayList<TLRPC.User> loadVCardFromStream(Uri uri, int currentAccount, boolean asset, ArrayList<VcardItem> items, String name) {
        ...
        try {
            InputStream stream;
            if (asset) {
                AssetFileDescriptor fd = BaseApplication.mApplicationContext.getContentResolver().openAssetFileDescriptor(uri, "r");
                stream = fd.createInputStream();
            } else {
                ContentResolver cr = BaseApplication.mApplicationContext.getContentResolver();
    ...
    public static void setWaitingForSms(boolean value) {
        synchronized (smsLock) {
            ...
            try {
                if (waitingForSms) {
                    SmsRetrieverClient client = SmsRetriever.getClient(BaseApplication.mApplicationContext);
    ...
    public static String[] getCurrentKeyboardLanguage() {
        try {
            InputMethodManager inputManager = (InputMethodManager) BaseApplication.mApplicationContext.getSystemService(Context.INPUT_METHOD_SERVICE);
    ...
    public static ArrayList<File> getDataDirs() {
        ...
        if (Build.VERSION.SDK_INT >= 19) {
            File[] dirs = BaseApplication.mApplicationContext.getExternalFilesDirs(null);
            if (dirs != null) {
    ...
    public static File getCacheDir() {
        ...
        if (state == null || state.startsWith(Environment.MEDIA_MOUNTED)) {
            try {
                File file;
                if (Build.VERSION.SDK_INT >= 19) {
                    File[] dirs = BaseApplication.mApplicationContext.getExternalCacheDirs();
                    file = dirs[0];
                    ...
                    file = BaseApplication.mApplicationContext.getExternalCacheDir();
                }
                if (file != null) {
                    return file;
                }
        ...
        try {
            File file = BaseApplication.mApplicationContext.getCacheDir();
            if (file != null) {
                return file;
            }
        } catch (Exception e) {
    ...
    public static void runOnUIThread(Runnable runnable, long delay) {
        if (BaseApplication.mApplicationHandler == null) {
            return;
        }
        if (delay == 0) {
    ...
    public static boolean isTablet() {
        if (isTablet == null) {
            isTablet = BaseApplication.mApplicationContext != null && BaseApplication.mApplicationContext.getResources().getBoolean(R.bool.isTablet);
        }
        return isTablet;
    }
    ...
    public static String obtainLoginPhoneCall(String pattern) {
        ...
        try (Cursor cursor = BaseApplication.mApplicationContext.getContentResolver().query(
    ...
    public static boolean needShowPasscode(boolean reset) {
        ...
        if (BaseBuildVars.LOGS_ENABLED && reset && SharedConfig.passcodeHash.length() > 0) {
    ...
    private static File getAlbumDir(boolean secretChat) { //TODO scoped storage
        if (secretChat || Build.VERSION.SDK_INT >= 23 && BaseApplication.mApplicationContext.checkSelfPermission(android.Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
    ...
    public static boolean openForView(File f, String fileName, String mimeType, final Activity activity) {
        if (f != null && f.exists()) {
            ...
            if (Build.VERSION.SDK_INT >= 26 && realMimeType != null && realMimeType.equals("application/vnd.android.package-archive") && !BaseApplication.mApplicationContext.getPackageManager().canRequestPackageInstalls()) {
                AlertDialog.Builder builder = new AlertDialog.Builder(activity);
                builder.setTitle(LocaleController.getString("AppName", R.string.AppName));
    ...
    public static boolean shouldEnableAnimation() {
        ...
        float scale = Settings.Global.getFloat(BaseApplication.mApplicationContext.getContentResolver(), Settings.Global.ANIMATOR_DURATION_SCALE, 1.0f);
        if (scale <= 0.0f) {
            return false;
        }
        return true;
...
///*        fragment.showDialog(new ShareAlert(fragment.getParentActivity(), null, url, false, url, false));*/
    ...
    public static String getCertificateSHA256Fingerprint() {
        PackageManager pm = BaseApplication.mApplicationContext.getPackageManager();
        String packageName = BaseApplication.mApplicationContext.getPackageName();
        try {
