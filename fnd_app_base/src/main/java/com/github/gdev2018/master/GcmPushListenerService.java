            ...
            Utilities.stageQueue.postRunnable(() -> {
                ...
                try {
                    ...
                    if (userIdObject == null) {
                        accountUserId = BaseUserConfig.getInstance(BaseUserConfig.selectedAccount).getClientUserId();
                    } else {
                    ...
                    for (int a = 0; a < BaseUserConfig.MAX_ACCOUNT_COUNT; a++) {
                        if (BaseUserConfig.getInstance(a).getClientUserId() == accountUserId) {
                            account = a;
                            break;
                        }
                    }
                    ...
                    switch (loc_key) {
                        ...
                        case "SESSION_REVOKE": {
                            AndroidUtilities.runOnUIThread(() -> {
                                if (BaseUserConfig.getInstance(accountFinal).getClientUserId() != 0) {
                                    BaseUserConfig.getInstance(accountFinal).clearConfig();
