...
package com.github.gdev2018.master.browser;
...
import com.github.gdev2018.master.CustomTabsCopyReceiver;
...
import com.github.gdev2018.master.support.customtabs.CustomTabsServiceConnection;
import com.github.gdev2018.master.support.customtabs.CustomTabsSession;
import com.github.gdev2018.master.support.customtabsclient.shared.CustomTabsHelper;
import com.github.gdev2018.master.support.customtabsclient.shared.ServiceConnection;
import com.github.gdev2018.master.support.customtabsclient.shared.ServiceConnectionCallback;
        ...
        if (tryTelegraph) {
            try {
                ...
                if (isTelegraphUrl(host, true) || uri.toString().toLowerCase().contains("telegram.org/faq") || uri.toString().toLowerCase().contains("telegram.org/privacy")) {
                    ...
                    final int reqId = ConnectionsManager.getInstance(BaseUserConfig.selectedAccount).sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
                        try {
                    ...
                    AndroidUtilities.runOnUIThread(() -> {
                        ...
                        try {
                            progressDialog[0].setOnCancelListener(dialog -> ConnectionsManager.getInstance(BaseUserConfig.selectedAccount).cancelRequest(reqId, true));
...
//            }*/
            if (allowCustom && SharedConfig.customTabs && !internalUri && !scheme.equals("tel")) {
                ...
                if (forceBrowser[0] || allActivities == null || allActivities.isEmpty()) {
                    Intent share = new Intent(BaseApplication.mApplicationContext, ShareBroadcastReceiver.class);
                    share.setAction(Intent.ACTION_SEND);

                    PendingIntent copy = PendingIntent.getBroadcast(BaseApplication.mApplicationContext, 0, new Intent(BaseApplication.mApplicationContext, CustomTabsCopyReceiver.class), PendingIntent.FLAG_UPDATE_CURRENT);
                    ...
                    builder.setActionButton(BitmapFactory.decodeResource(context.getResources(), R.drawable.abc_ic_menu_share_mtrl_alpha), LocaleController.getString("ShareFile", R.string.ShareFile), PendingIntent.getBroadcast(BaseApplication.mApplicationContext, 0, share, 0), true);
...
//                intent.setComponent(componentName);*/
    ...
    public static boolean isInternalUri(Uri uri, boolean all, boolean[] forceBrowser) {
        ...
        if ("ton".equals(uri.getScheme())) {
            try {
                Intent viewIntent = new Intent(Intent.ACTION_VIEW, uri);
                List<ResolveInfo> allActivities = BaseApplication.mApplicationContext.getPackageManager().queryIntentActivities(viewIntent, 0);
