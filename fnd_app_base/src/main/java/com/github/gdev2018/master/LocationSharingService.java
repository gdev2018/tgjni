...
public class LocationSharingService extends Service implements NotificationCenter.NotificationCenterDelegate {
    ...
    private void updateNotification(boolean post) {
        ...
        if (post) {
            NotificationManagerCompat.from(BaseApplication.mApplicationContext).notify(6, builder.build());
        }
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (builder == null) {
///*            Intent intent2 = new Intent(BaseApplication.mApplicationContext, LaunchActivity.class);*/
            Intent intent2 = new Intent(BaseApplication.mApplicationContext, Activity.class);
            intent2.setAction("org.tmessages.openlocations");
            intent2.addCategory(Intent.CATEGORY_LAUNCHER);
            PendingIntent contentIntent = PendingIntent.getActivity(BaseApplication.mApplicationContext, 0, intent2, 0);

            builder = new NotificationCompat.Builder(BaseApplication.mApplicationContext);
            ...
            Intent stopIntent = new Intent(BaseApplication.mApplicationContext, StopLiveLocationReceiver.class);
            builder.addAction(0, LocaleController.getString("StopLiveLocation", R.string.StopLiveLocation), PendingIntent.getBroadcast(BaseApplication.mApplicationContext, 2, stopIntent, PendingIntent.FLAG_UPDATE_CURRENT));
        }
