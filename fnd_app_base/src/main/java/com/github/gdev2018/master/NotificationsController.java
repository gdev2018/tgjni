...
///*import com.github.gdev2018.master.ui.PopupNotificationActivity;*/
...
public class NotificationsController {
    ...
    static {
        if (Build.VERSION.SDK_INT >= 26 && BaseApplication.mApplicationContext != null) {
            notificationManager = NotificationManagerCompat.from(BaseApplication.mApplicationContext);
            systemNotificationManager = (NotificationManager) BaseApplication.mApplicationContext.getSystemService(Context.NOTIFICATION_SERVICE);
            checkOtherNotificationsChannel();
        }
        audioManager = (AudioManager) BaseApplication.mApplicationContext.getSystemService(Context.AUDIO_SERVICE);
    }
    ...
    public NotificationsController(int instance) {
        ...
        try {
            alarmManager = (AlarmManager) BaseApplication.mApplicationContext.getSystemService(Context.ALARM_SERVICE);
        } catch (Exception e) {
            FileLog.e(e);
        }

        try {
            PowerManager pm = (PowerManager) BaseApplication.mApplicationContext.getSystemService(Context.POWER_SERVICE);
///*            notificationDelayWakelock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "lock");*/
    ...
    public static void checkOtherNotificationsChannel() {
        ...
        if (OTHER_NOTIFICATIONS_CHANNEL == null) {
            preferences = BaseApplication.mApplicationContext.getSharedPreferences("Notifications", Activity.MODE_PRIVATE);
...
///*                    Intent popupIntent = new Intent(BaseApplication.mApplicationContext, PopupNotificationActivity.class);
...
//                    BaseApplication.mApplicationContext.startActivity(popupIntent);
//                    Intent it = new Intent(Intent.ACTION_CLOSE_SYSTEM_DIALOGS);
//                    BaseApplication.mApplicationContext.sendBroadcast(it);*/
                });
            }
        });
    }
...
//                if (dialog_id == opened_dialog_id && BaseApplication.isScreenOn) {
//                    if (!isFcm) {
//                        playInChatSound();
//                    }
//                    continue;
...
//*/
            ...
            if (!popupArrayAdd.isEmpty() && !AndroidUtilities.needShowPasscode(false)) {
                ...
                AndroidUtilities.runOnUIThread(() -> {
                    ...
                    if (BaseApplication.mainInterfacePaused || !BaseApplication.isScreenOn && !SharedConfig.isWaitingForPasscodeEnter) {
                        if (popupFinal == 3 || popupFinal == 1 && BaseApplication.isScreenOn || popupFinal == 2 && !BaseApplication.isScreenOn) {
///*                            Intent popupIntent = new Intent(BaseApplication.mApplicationContext, PopupNotificationActivity.class);
//                            popupIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_NO_ANIMATION | Intent.FLAG_ACTIVITY_NO_USER_ACTION | Intent.FLAG_FROM_BACKGROUND);
//                            try {
//                                BaseApplication.mApplicationContext.startActivity(popupIntent);
...
//                        settingsCache.put(dialog_id, value);
//                    }
//                    if (!value || dialog_id == opened_dialog_id && BaseApplication.isScreenOn) {
//                        continue;
//                    }
//                    pushMessagesDict.put(mid, messageObject);
//                    pushMessages.add(0, messageObject);
...
//                from_id = -chat_id;
//            }
//        } else if (from_id == BaseUserConfig.getInstance(currentAccount).getClientUserId()) {
//            from_id = messageObject.messageOwner.from_id;
//        }
//
//        if (dialog_id == 0) {
...
//                        return LocaleController.formatString("NotificationUnrecognizedDevice", R.string.NotificationUnrecognizedDevice, BaseUserConfig.getInstance(currentAccount).getCurrentUser().first_name, date, messageObject.messageOwner.action.title, messageObject.messageOwner.action.address);
...
//                            } else {
//                                if (singleUserId == BaseUserConfig.getInstance(currentAccount).getClientUserId()) {
...
//                        if (messageObject.messageOwner.action.user_id == BaseUserConfig.getInstance(currentAccount).getClientUserId()) {
...
//                        } else if (messageObject.messageOwner.action instanceof TLRPC.TL_messageActionLoginUnknownLocation) {
//                            String date = LocaleController.formatString("formatDateAtTime", R.string.formatDateAtTime, LocaleController.getInstance().formatterYear.format(((long) messageObject.messageOwner.date) * 1000), LocaleController.getInstance().formatterDay.format(((long) messageObject.messageOwner.date) * 1000));
//                            msg = LocaleController.formatString("NotificationUnrecognizedDevice", R.string.NotificationUnrecognizedDevice, BaseUserConfig.getInstance(currentAccount).getCurrentUser().first_name, date, messageObject.messageOwner.action.title, messageObject.messageOwner.action.address);
//                        } else if (messageObject.messageOwner.action instanceof TLRPC.TL_messageActionGameScore || messageObject.messageOwner.action instanceof TLRPC.TL_messageActionPaymentSent) {
...
//        return msg;
    ...
    private void scheduleNotificationRepeat() {
        try {
            Intent intent = new Intent(BaseApplication.mApplicationContext, NotificationRepeat.class);
            intent.putExtra("currentAccount", currentAccount);
            PendingIntent pintent = PendingIntent.getService(BaseApplication.mApplicationContext, 0, intent, 0);
            SharedPreferences preferences = MessagesController.getNotificationsSettings(currentAccount);
        ...
        try {
            ...
            if (WearDataLayerListenerService.isWatchConnected()) {
                try {
                    ...
                    o.put("id", BaseUserConfig.getInstance(currentAccount).getClientUserId());
            ...
            notificationsQueue.postRunnable(() -> {
                ...
                try {
                    ...
                    if (soundIn == 0 && !soundInLoaded) {
                        soundInLoaded = true;
                        soundIn = soundPool.load(BaseApplication.mApplicationContext, R.raw.sound_in, 1);
                    }
                    if (soundIn != 0) {
                        try {
    ...
    private String validateChannelId(long dialogId, String name, long[] vibrationPattern, int ledColor, Uri sound, int importance, long[] configVibrationPattern, Uri configSound, int configImportance) {
        SharedPreferences preferences = MessagesController.getNotificationsSettings(currentAccount);
        String key = "com.github.gdev2018.master.key" + dialogId;
            ...
            if (!settings.equals(newSettingsHash)) {
                ...
                if (configSound == null || channelSound != null || configSound != null && channelSound == null || !configSound.equals(channelSound)) {
                    ...
                    if (channelSound == null) {
                        ...
                        Ringtone rng = RingtoneManager.getRingtone(BaseApplication.mApplicationContext, channelSound);
                        ...
                        if (rng != null) {
                            if (channelSound.equals(Settings.System.DEFAULT_RINGTONE_URI)) {
                                ...
                                ringtoneName = rng.getTitle(BaseApplication.mApplicationContext);
                            }
                            rng.stop();
                        }
    ...
    private void showOrUpdateNotification(boolean notifyAboutLast) {
        if (!BaseUserConfig.getInstance(currentAccount).isClientActivated() || pushMessages.isEmpty()) {
            dismissNotification();
            return;
        }
//        try {
...
//            } else if (user_id == BaseUserConfig.getInstance(currentAccount).getClientUserId()) {
//                user_id = lastMessageObject.messageOwner.from_id;
//            }
//
...
//            }
//
/////*            Intent intent = new Intent(BaseApplication.mApplicationContext, LaunchActivity.class);*/
//            Intent intent = new Intent(BaseApplication.mApplicationContext, Activity.class);
...
//            }
//            intent.putExtra("currentAccount", currentAccount);
//            PendingIntent contentIntent = PendingIntent.getActivity(BaseApplication.mApplicationContext, 0, intent, PendingIntent.FLAG_ONE_SHOT);
//            String name;
//            String chatName;
...
//
//            String detailText;
//            if (BaseUserConfig.getActivatedAccountsCount() > 1) {
//                if (pushDialogs.size() == 1) {
//                    detailText = UserObject.getFirstName(BaseUserConfig.getInstance(currentAccount).getCurrentUser());
//                } else {
//                    detailText = UserObject.getFirstName(BaseUserConfig.getInstance(currentAccount).getCurrentUser()) + "・";
//                }
//            } else {
...
//            }
//
//            NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(BaseApplication.mApplicationContext)
...
//            }
//
//            Intent dismissIntent = new Intent(BaseApplication.mApplicationContext, NotificationDismissReceiver.class);
...
//            mBuilder.setDeleteIntent(PendingIntent.getBroadcast(BaseApplication.mApplicationContext, 1, dismissIntent, PendingIntent.FLAG_UPDATE_CURRENT));
//
//            if (photoPath != null) {
...
//                if (BaseApplication.mainInterfacePaused || inAppPreview) {
...
//                                Intent callbackIntent = new Intent(BaseApplication.mApplicationContext, NotificationCallbackReceiver.class);
...
//                                mBuilder.addAction(0, button.text, PendingIntent.getBroadcast(BaseApplication.mApplicationContext, lastButtonId++, callbackIntent, PendingIntent.FLAG_UPDATE_CURRENT));
//                                hasCallback = true;
//                            }
//                        }
//                    }
...
//                Intent replyIntent = new Intent(BaseApplication.mApplicationContext, PopupReplyReceiver.class);
//                replyIntent.putExtra("currentAccount", currentAccount);
//                if (Build.VERSION.SDK_INT <= 19) {
//                    mBuilder.addAction(R.drawable.ic_ab_reply2, LocaleController.getString("Reply", R.string.Reply), PendingIntent.getBroadcast(BaseApplication.mApplicationContext, 2, replyIntent, PendingIntent.FLAG_UPDATE_CURRENT));
//                } else {
//                    mBuilder.addAction(R.drawable.ic_ab_reply, LocaleController.getString("Reply", R.string.Reply), PendingIntent.getBroadcast(BaseApplication.mApplicationContext, 2, replyIntent, PendingIntent.FLAG_UPDATE_CURRENT));
//                }
//            }
//            if (Build.VERSION.SDK_INT >= 26) {
...
///*                            isSupergroup = lastMessageObject.isMegagroup();*/
            ...
            Intent msgHeardIntent = new Intent(BaseApplication.mApplicationContext, AutoMessageHeardReceiver.class);
            msgHeardIntent.addFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
            msgHeardIntent.setAction("com.github.gdev2018.master.ACTION_MESSAGE_HEARD");
            ...
            PendingIntent msgHeardPendingIntent = PendingIntent.getBroadcast(BaseApplication.mApplicationContext, internalId, msgHeardIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            if ((!isChannel || isSupergroup) && canReply && !SharedConfig.isWaitingForPasscodeEnter) {
                Intent msgReplyIntent = new Intent(BaseApplication.mApplicationContext, AutoMessageReplyReceiver.class);
                msgReplyIntent.addFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
                msgReplyIntent.setAction("com.github.gdev2018.master.ACTION_MESSAGE_REPLY");
                ...
                PendingIntent msgReplyPendingIntent = PendingIntent.getBroadcast(BaseApplication.mApplicationContext, internalId, msgReplyIntent, PendingIntent.FLAG_UPDATE_CURRENT);
                ...
                Intent replyIntent = new Intent(BaseApplication.mApplicationContext, WearReplyReceiver.class);
                replyIntent.putExtra("dialog_id", dialog_id);
                replyIntent.putExtra("max_id", max_id);
                replyIntent.putExtra("currentAccount", currentAccount);
                PendingIntent replyPendingIntent = PendingIntent.getBroadcast(BaseApplication.mApplicationContext, internalId, replyIntent, PendingIntent.FLAG_UPDATE_CURRENT);
...
//                            }*/
                ...
                if (lowerId != 0) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P && !((ActivityManager) BaseApplication.mApplicationContext.getSystemService(Context.ACTIVITY_SERVICE)).isLowRamDevice()) {
                        if (messageObject.type == 1 || messageObject.isSticker()) {
                            ...
                            final Uri uri;
                            if (attach.exists()) {
                                uri = FileProvider.getUriForFile(BaseApplication.mApplicationContext, BuildConfig.APPLICATION_ID + ".provider", attach);
                            ...
                            if (uri != null) {
                                ...
                                BaseApplication.mApplicationContext.grantUriPermission("com.android.systemui", uri, Intent.FLAG_GRANT_READ_URI_PERMISSION);
                                AndroidUtilities.runOnUIThread(() -> BaseApplication.mApplicationContext.revokeUriPermission(uri, Intent.FLAG_GRANT_READ_URI_PERMISSION), 20000);
                    ...
                    if (messageObject.isVoice()) {
                        ...
                        if (!messages.isEmpty()) {
                            ...
                            if (Build.VERSION.SDK_INT >= 24) {
                                try {
                                    uri = FileProvider.getUriForFile(BaseApplication.mApplicationContext, BuildConfig.APPLICATION_ID + ".provider", f);
...
///*            Intent intent = new Intent(BaseApplication.mApplicationContext, LaunchActivity.class);*/
            ...
            PendingIntent readPendingIntent = PendingIntent.getBroadcast(BaseApplication.mApplicationContext, internalId, msgHeardIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            wearableExtender.setBridgeTag("tgaccount" + BaseUserConfig.getInstance(currentAccount).getClientUserId());

            long date = ((long) messageObjects.get(0).messageOwner.date) * 1000;

            NotificationCompat.Builder builder = new NotificationCompat.Builder(BaseApplication.mApplicationContext)
                    .setContentTitle(name)
                    .setSmallIcon(R.drawable.notification)
                ...
                for (int r = 0, rc = rows.size(); r < rc; r++) {
                    ...
                    for (int c = 0, cc = row.buttons.size(); c < cc; c++) {
                        ...
                        if (button instanceof TLRPC.TL_keyboardButtonCallback) {
                            ...
                            builder.addAction(0, button.text, PendingIntent.getBroadcast(BaseApplication.mApplicationContext, lastButtonId++, callbackIntent, PendingIntent.FLAG_UPDATE_CURRENT));
                        }
                    }
                }
            }
        ...
        if (serializedNotifications != null) {
            try {
                ...
                s.put("id", BaseUserConfig.getInstance(currentAccount).getClientUserId());
        ...
        notificationsQueue.postRunnable(() -> {
            try {
                ...
                if (soundOut == 0 && !soundOutLoaded) {
                    soundOutLoaded = true;
                    soundOut = soundPool.load(BaseApplication.mApplicationContext, R.raw.sound_out, 1);
                }
                if (soundOut != 0) {
                    try {
