...
//*/
    ...
    public static class ThemeAccent {
        ...
        public File getPathToWallpaper() {
            if (id < 100) {
                return !TextUtils.isEmpty(patternSlug) ? new File(BaseApplication.getFilesDirFixed(), String.format(Locale.US, "%s_%d_%s_v3.jpg", parentTheme.getKey(), id, patternSlug)) : null;
            } else {
                    ...
                    if (accent != null && theme.settings != null) {
                        if (!ThemeInfo.accentEquals(accent, theme.settings)) {
                            ...
                            if (currentTheme == themeInfo && currentTheme.currentAccentId == accent.id) {
                                refreshThemeColors();
                                createChatResources(BaseApplication.mApplicationContext, false);
